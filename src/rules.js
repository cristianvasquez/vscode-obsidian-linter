 ...
 */
export function getDisabledRules (text) {
  ...
  let disabled_rules = (parsed_yaml)
    [
    'disabled rules'
    ]
  if (!disabled_rules) {
...
export class Rule {
  ...
  constructor (
    ...
    options.unshift(new Option(this.description, '', false))
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          switch (options['Style']) {
            case 'Title Case': {
              ...
              const keepCasing = (options['Ignore Words']

              ).split(
                /[,\s]+/
              )
              const ignoreShortWords = (
                options['Lowercase Words']
